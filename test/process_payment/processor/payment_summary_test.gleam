...
pub fn stores_concurrent_payment_information_test() {
  ...
  let payments_finished_count = booklet.new(0)

  // just getting the sum we expect to see on the payments total at the end
  let expected_payment_total =
    payments |> list.map(fn(payment) { payment.amount }) |> float.sum
  ...
  payments
  |> list.each(fn(_payment) {
    use <- process.spawn

    booklet.update(payments_finished_count, fn(x) { x + 1 })
    process.send(completion_subject, Nil)
  })

  // here we just force the test case process to wait for a message related to each payment. for example,
  // if the test generates 1030 payments, receive_forever will be called 1030 times, which means it'll only move to the
  ...
  let count = booklet.get(payments_finished_count)
